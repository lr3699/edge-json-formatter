...
一个 Microsoft Edge / Chrome 扩展（Manifest V3）：自动识别网页返回的 JSON，替换成带语法高亮、可折叠树的查看器；内置双栏编辑页，粘贴自己复制的 JSON 也能即时格式化。
...
| 可折叠树 | 格式化后默认全展开；逐节点可折叠、折叠摘要（`… 13 个字段`）；超大 JSON 分批渲染 + 「加载更多」 |
...
| --- | --- |
| 自动接管 | 打开 JSON 页面即可 |
| 点扩展图标 | 直接打开「粘贴 JSON 格式化」编辑页（无二级弹窗） |
| 右键菜单 | 页面空白处右键 → 格式化当前页面 / 格式化选中的内容 / 打开编辑页 |
...
- **右栏**：和接管网页时完全同一个查看器 —— 折叠树、语法高亮、路径复制、导出。
