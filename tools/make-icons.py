...
为什么分档（2026-09-23 实测结论）：
    ...
    → **16/32 改用单字母「J」加粗做主形；48px 起才用完整字标「JSON」+ 角括号。**
...
# 小尺寸描边加粗量（viewBox 单位）：16px 下 26/512 ≈ 0.81px 额外笔画。
# 只给单字母补——字母笔画本身偏细；花括号那类重笔画字形切勿描边，会糊成白块。
STROKE_SMALL = {16: 26, 32: 12}
...
SITE_FAVICON_STROKE = STROKE_SMALL[16]
...
def marks_for(size, M, force_small=None, stroke=None):
    ...
    if small:
        ...
        stroke = STROKE_SMALL.get(size, 0) if stroke is None else stroke
...
def applied_page(pairs, fav_rows):
    ...
    return ('<!DOCTYPE html><html lang="zh-CN"><head><meta charset="utf-8">'
            ...
            '改后：<b>16/32px 换成加粗单字母 J</b>（四字母在此尺寸墨迹仅约 3 物理像素，'
            '必然糊），48px 起保持完整字标 <b>「JSON」</b>。</p>%s'
            '<div class="split"></div>'
...
def main():
    ...
    page = ('<!DOCTYPE html><html lang="zh-CN"><head><meta charset="utf-8">'
            ...
            '<b>16px / 32px</b> 用加粗单字母「%s」（墨迹高 %.0f%% 版面）——四字母字标在 16px 下'
